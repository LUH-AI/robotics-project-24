...
from .legged_robot_config_2 import LeggedRobotV2Cfg
from legged_gym.envs.base.low_level_action_manager import ActionManager

class LeggedRobot_v2(BaseTask):
    def __init__(self, cfg: LeggedRobotV2Cfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.init_done = True
        #self.action_manger = ActionManager(self.device, self.cfg)
    ...
    def step(self, actions):
        ...
        #self.action_manager.process_action(self.action.to(self.device)) ###
    ...
    def post_physics_step(self):
        ...
        self.compute_low_level_observations() # in some cases a simulation step might be required to refresh some obs (for example body positions)
        self.compute_observations() #computes high level observations
    ...
    def compute_low_level_observations(self):
        ...
        # add noise if needed
        #only necessary when we train low_level_policy
        """ if self.add_noise:
    ...
    def _init_buffers(self):
        ...
        self.actions = torch.zeros(self.num_envs, 3, dtype=torch.float, device=self.device, requires_grad=False)
        self.last_actions = torch.zeros(self.num_envs, 3, dtype=torch.float, device=self.device, requires_grad=False)
